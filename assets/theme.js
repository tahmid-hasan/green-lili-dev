...
(function(){
  ...
  theme.LocaleDrawer = (function () {
    const selectors = {
      drawer: '#LocaleDrawer',
      form: '#LocaleDrawerForm'
    }

    function LocaleDrawer() {
      this.form = document.querySelector(selectors.form)
      this.drawer = new theme.Drawers('LocaleDrawer', 'localization')
      
      this.init()
    }

    LocaleDrawer.prototype = Object.assign({}, LocaleDrawer.prototype, {
      init: function() {

        this.form.addEventListener('change', this.handleLocaleChange.bind(this))

        // Dev-friendly way to open cart
        document.addEventListener('localization:open', this.open.bind(this));
        document.addEventListener('localization:close', this.close.bind(this));
      },

      open: function() {
        this.drawer.open()
        console.log('locale-open')

        theme.a11y.trapFocus({
          container: this.form,
          elementToFocus: this.form,
          namespace: 'localedrawer_focus'
        })
      },
      ...
      reRenderLocaleSelector: function(market) {
  ...
  theme.Product = (function() {
    ...
    function Product(container) {
      ...
      this.selectors = {
        ...
        // for size selector start
        sizeLabel: '[data-variant-size-label]',
        variantSizeOption: '.variant__input--size-option',
        // for size selector end
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      initVariants: function() {
        ...
        // for size selector start
        var sizes = this.container.querySelectorAll(this.selectors.variantSizeOption);
        if (sizes.length) {
          sizes.forEach(size => {
            size.addEventListener('change', function(evt) {
              var color = size.dataset.sizeName;
              var index = size.dataset.sizeIndex;
              this.updateSizeName(color, index);
            }.bind(this))
          });
        }
        // for size selector end
